  ...
  const getRankingsForPeriod = (periodMatches: Match[], useStars: boolean = false) => {
    ...
    periodMatches.forEach(m => {
      ...
      pIds.forEach(id => {
        ...
        // Add stars from match
        if (m.stars && m.stars[id]) {
          stats[id].stars += m.stars[id];
        }
        // Track double star matches
        if (m.isDoubleStar) {
    ...
    return players
      .filter(p => useStars ? stats[p.id]?.stars > 0 : stats[p.id]?.total > 0)
      .map(p => {
        const s = stats[p.id];
        return {
          ...p,
          ...
          pointDiff: s.diff,
          stars: s.stars,
          doubleStarMatches: s.doubleStarMatches
        };
      })
      .sort((a, b) => {
        if (useStars) {
