...
import { Users, Search, Filter, ArrowUpDown, Loader2 } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const profiles = profilesSnap.docs.map(d => ({ ...d.data(), id: d.id }));
        setAllUserProfiles(profiles);

        // Fetch all matches from all sessions
        try {
          ...
          console.warn('Collection group query failed, trying individual sessions:', err);
          // Fallback: try to get matches from all sessions individually
          const sessionsSnap = await getDocs(collection(firestore, 'sessions'));
          const allMatchesArray: any[] = [];
          for (const sessionDoc of sessionsSnap.docs) {
            try {
              const sessionMatchesQuery = query(
                collection(firestore, 'sessions', sessionDoc.id, 'matches'),
                where('status', '==', 'completed')
              );
              const sessionMatchesSnap = await getDocs(sessionMatchesQuery);
              sessionMatchesSnap.docs.forEach(d => {
                allMatchesArray.push({ ...d.data(), id: d.id, sessionId: sessionDoc.id });
              });
            } catch (e) {
              console.warn(`Failed to fetch matches for session ${sessionDoc.id}:`, e);
            }
          }
          setAllMatches(allMatchesArray);
        }
      } catch (err) {
        ...
        <Card>
          <CardContent className="flex items-center justify-center py-20">
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-sm font-black uppercase text-muted-foreground">Loading players...</p>
            </div>
          </CardContent>
        </Card>
      ) : (
